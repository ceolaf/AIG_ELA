...
import re                                        # for matching standard directories
...
def SelectPassage() -> tuple[str, str]:
...
def PrintToFileAndScreen(LLM: str, tier_code: str, index: int, response: str, 
    file_name: str, passage_name: str, 
    elapsed: float, input_tokens: int, output_tokens: int) -> None:
    ...
    with open(file_name, "a") as f:
        ...
        f.write(f"({elapsed:.2f} secs). ({input_tokens} + {output_tokens} = {total_tokens} total tokens)\n\n")
        f.write(response)
...
def RunGPT(tiers: list[tuple[str, str, str]], item_per_tier: int, passage_name: str) -> None:
    print(f"\n\n********************* Starting GPT tiers... ")
    ...
    for tier_code, prompt_text, file_name in tiers:
        ...
        PrintToFileAndScreen("GPT", tier_code, 1, response.output_text, file_name, passage_name, 
        ...
        for index in range(2, item_per_tier + 1):
            ...
            # Call AMH's printing/filing function
            PrintToFileAndScreen("GPT", tier_code, index, response.output_text, file_name, passage_name, 
                elapsed, response.usage.input_tokens, response.usage.output_tokens)
...
def RunGemini(tiers: list[tuple[str, str, str]], item_per_tier: int, passage_name: str) -> None:
    ...
    for tier_code, prompt_text, file_name in tiers:
        try:
            ...
            # Call AMH's printing/filing function
            PrintToFileAndScreen("Gemini", tier_code, 1, response.text, file_name, passage_name, elapsed, 
            ...
            for index in range(2, item_per_tier + 1):
                ...
                # Call AMH's printing/filing function
                PrintToFileAndScreen("Gemini", tier_code, index, response.text, file_name, passage_name, elapsed, 
                    (response.usage_metadata or {}).get("prompt_token_count", 0),
                    (response.usage_metadata or {}).get("candidates_token_count", 0) )
...
def RunClaude(tiers: list[tuple[str, str, str]], item_per_tier: int, passage_name: str) -> None:
    ...
    for tier_code, prompt_text, file_name in tiers:
        ...
        # Call AMH's printing/filing function
        PrintToFileAndScreen("Claude", tier_code, 1, response_text, file_name, passage_name, elapsed,
        ...
        for index in range(2, item_per_tier + 1):
            ...
            # Call AMH's printing/filing function
            PrintToFileAndScreen("Claude", tier_code, index, response_text, file_name, passage_name, 
                elapsed, response.usage.input_tokens, response.usage.output_tokens)
...
def RunCopilot(tiers: list[tuple[str, str, str]], item_per_tier: int, passage_name: str) -> None:
    ...
    with httpx.Client(timeout=60.0) as client:
        ...
        for tier_code, prompt_text, file_name in tiers:
            ...
            PrintToFileAndScreen("Copilot", tier_code, 1, first_text, file_name, passage_name, 
                elapsed, 0, 0)
            ...
            for index in range(2, item_per_tier + 1):
                ...
                PrintToFileAndScreen("Copilot", tier_code, index, next_text, file_name, passage_name, 
                    elapsed, 0, 0)
                time.sleep(1)
