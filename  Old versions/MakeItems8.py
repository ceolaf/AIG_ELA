...
def PrintToFileAndScreen(LLM, tier_code, index, response, file_name):
...
def RunGPT(tiers, item_per_tier):
...
def RunGemini(tiers, item_per_tier, model_name='gemini-2.5-pro'):
...
def RunClaude(tiers, item_per_tier):
    for tier_code, prompt_text, file_name in tiers:
...
def RunCopilot(tiers, item_per_tier):
    ...
    with httpx.Client(timeout=60.0) as client:
        ...
        for tier_code, prompt_text, file_name in tiers:
            ...
            print(f"Conversation create status: {conv_resp.status_code}")
            print(f"Conversation create content: {conv_resp.text}")

            if conv_resp.status_code != 201:
                print(f"Error creating conversation for tier {tier_code}, skipping.")
                continue

            conv_data = conv_resp.json()
            conversation_id = conv_data.get("id")
            if not conversation_id:
                print(f"No conversation ID returned for tier {tier_code}, skipping.")
                continue

            # Append a header for this new run (this is your existing behavior)
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            with open(file_name, "a") as f:
                f.write(
            ...
            # Helper to send a chat message and get the last text
            def send_chat(prompt: str, item_index: int) -> str | None:
                chat_payload = {
                    "message": {"text": prompt},
                ...
                print(
                    f"Chat status (tier {tier_code} #{item_index}): "
                    f"{chat_resp.status_code}"
                )
                print(
                    f"Chat raw response (tier {tier_code} #{item_index}): "
                    f"{chat_resp.text}"
                )

                # If things go wrong, you'll see the raw body here
                if chat_resp.status_code != 200:
                    print(
                        f"Chat error for tier {tier_code} #{item_index}, "
                ...
                try:
                    ...
                    print(
                        f"ERROR: Response is not valid JSON for "
                        f"tier {tier_code} #{item_index}."
                    )
                    return None
                ...
                if not messages:
                    print(
                        f"ERROR: No 'messages' field in Copilot chat response "
                        f"for tier {tier_code} #{item_index}."
                    )
                    return None
            ...
            first_text = send_chat(prompt_text, 1)
            if first_text is None:
                print(
                    f"Skipping tier {tier_code} due to chat error on first item."
                )
                continue
            ...
            for index in range(2, item_per_tier + 1):
                ...
                time.sleep(1)  # very light rate limiting
