...
# Written by GPT. 
def RunGemini(Tiers, ItemsPerTier):
    for TierCode, PromptText, FileName in Tiers:

        # Mark a new Gemini run (no overwrite)
        with open(FileName, "a") as f:
            f.write("\n\n========== NEW RUN (Gemini Pro) ==========\n\n")

        # ---- start a fresh chat for EACH tier (reset context per tier) ----
        Chat = GeminiClient.chats.create(model="gemini-2.5-pro")
        ...
        with open(FileName, "a") as f:
            f.write("\n\n========== NEW ITEM ==========\n\n")
            f.write(f"\nGemini Pro Tier {TierCode} #1\n")
            f.write(Response.text)

        print(Response.text)
        print(f"Gemini Pro {TierCode} #1 complete, saved to {FileName}")

        # If only 1 item needed, skip follow-up generation
        if ItemsPerTier == 1:
            continue
        ...
        for Index in range(2, ItemsPerTier + 1):
            ...
            with open(FileName, "a") as f:
                f.write("\n\n========== NEW ITEM ==========\n\n")
                f.write(f"\nGemini Pro Tier {TierCode} #{Index}\n")
                f.write(Response.text)

            print(Response.text)
            print(f"Gemini Pro {TierCode} #{Index} complete, saved to {FileName}")
